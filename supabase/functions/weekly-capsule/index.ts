    ...
    const response = await fetch("https://api.xiaomimimo.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are LEXOR®, an elite AI stylist. Generate a 7-day capsule wardrobe plan using ONLY items from the user's closet.
...
- The 7 days are: ${days.join(", ")}`,
    ...
    if (toolCall) {
      ...
      if (jsonMatch) {
        ...
        throw new Error("No weekly plan returned from AI");
      }
    }
