      ...
      <div className="w-full grid grid-cols-3 gap-4 items-end">
        <div className="col-span-2 flex flex-col justify-end h-full">
          <div className="space-y-2">
            <motion.h2 className="text-xl font-bold leading-tight">{title}</motion.h2>
            <motion.p className="text-sm opacity-80">{author}</motion.p>
          </div>
          <motion.button
            onClick={handleLikeClick}
            className={cn(
              'mt-4 flex items-center gap-2 w-fit px-4 py-2 rounded-full text-sm font-semibold transition-colors duration-300',
              isLiked ? 'bg-red-500/80 text-white' : 'bg-white/20 text-white backdrop-blur-sm hover:bg-white/30'
            )}
          >
            <motion.div whileTap={{ scale: 1.3 }}>
              <Heart className={cn('w-5 h-5 transition-all', isLiked ? 'fill-current' : 'fill-transparent')} />
            </motion.div>
            <AnimatePresence mode="wait">
              <motion.span key={likes} initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 10, opacity: 0 }} transition={{ duration: 0.2 }} className="w-10 text-left">
                {formatLikes(likes)}
              </motion.span>
            </AnimatePresence>
          </motion.button>
