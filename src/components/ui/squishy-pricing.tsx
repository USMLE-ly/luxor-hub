  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-white">
        <span className="mb-3 block w-fit rounded-full bg-black/30 backdrop-blur-md px-4 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-white border border-white/20 font-sans">
          {label}
        </span>
        <motion.div
          initial={{ scale: 0.85 }}
        ...
        >
          ...
          <span className="text-sm font-sans text-white/80 ml-1">/month</span>
        </motion.div>
        <p className="inline text-xs sm:text-sm text-white mt-1 font-sans leading-relaxed"><span className="bg-black/25 rounded px-1.5 py-0.5 box-decoration-clone">{description}</span></p>
      </div>
      ...
      {hasFeatures && (
        <>
          ...
          <ul className="relative z-10 space-y-2 flex-1">
            {features.map((f, i) => (
              <li key={i} className="flex items-start gap-2.5 text-white">
                <div className="w-4 h-4 mt-0.5 flex-shrink-0 rounded-full bg-black/25 flex items-center justify-center">
                  <Check className="w-2.5 h-2.5 text-white" strokeWidth={3} />
                </div>
                <span className="text-xs font-sans leading-relaxed tracking-wide bg-black/25 rounded px-1.5 py-0.5 box-decoration-clone">{f}</span>
              </li>
            ))}
          </ul>
        </>
