...
const FaceShapeIllustration = ({ shape, size = 120, className = "" }: Props) => {
  const key = getShapeKey(shape);
  const path = shapeOutlines[key] || shapeOutlines.oval;
  ...
  return (
    ...
    >
      <svg width={size} height={size} viewBox="0 0 100 100" fill="none">
        <defs>
          <linearGradient id={`face-fill-${key}`} x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="hsl(25, 60%, 88%)" />
            <stop offset="100%" stopColor="hsl(15, 50%, 80%)" />
          </linearGradient>
          <linearGradient id={`face-stroke-${key}`} x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <motion.path
          d={path}
          fill={`url(#face-fill-${key})`}
          stroke={`url(#face-stroke-${key})`}
          strokeWidth="2"
