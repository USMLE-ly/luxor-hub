      ...
      {items.map((item, index) => (
        <div
          ...
          className={cn(
            ...
            "hover:shadow-[0_2px_12px_hsl(var(--primary)/0.1)]",
            "hover:-translate-y-0.5 will-change-transform",
            item.colSpan === 2 ? "md:col-span-2" : "col-span-1",
            {
              "shadow-[0_2px_12px_hsl(var(--primary)/0.05)] -translate-y-0.5":
        ...
        >
            ...
            <div className="flex items-center justify-between mt-2">
              <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                {item.tags?.map((tag, i) => (
                  <span
                    key={i}
                    className="px-2 py-1 rounded-md bg-muted backdrop-blur-sm transition-all duration-200 hover:bg-primary/10"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          <div
            className={`absolute inset-0 -z-10 rounded-xl p-px bg-gradient-to-br from-transparent via-border/50 to-transparent ${
              item.hasPersistentHover
                ? "opacity-100"
                : "opacity-0 group-hover:opacity-100"
            } transition-opacity duration-300`}
