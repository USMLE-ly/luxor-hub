...
export const LiquidGlassCard = ({
  ...
  const blurClasses = {
    sm: 'backdrop-blur-sm', md: 'backdrop-blur-md', lg: 'backdrop-blur-lg', xl: 'backdrop-blur-xl',
  };
  ...
  const motionProps = draggable || expandable ? {
    ...
    dragElastic: 0.3,
    dragTransition: { bounceStiffness: 300, bounceDamping: 10, power: 0.3 },
  ...
  return (
    <>
      ...
      <MotionComponent
        className={cn(`relative ${draggable ? 'cursor-grab active:cursor-grabbing' : ''} ${expandable ? 'cursor-pointer' : ''}`, className)}
        style={{ borderRadius, ...(width && !expandable && { width }), ...(height && !expandable && { height }) }}
        {...motionProps}
        {...props}
      >
