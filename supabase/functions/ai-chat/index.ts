...
serve(async (req) => {
  ...
  try {
    // ── TENANT ISOLATION: Extract userId from JWT, NOT from request body ──
    ...
    const { messages, styleProfile, closetSummary, image, mood } = await req.json();
    ...
    const systemPrompt = `You are LEXOR®, an elite AI personal stylist. You speak with warmth, confidence, and deep fashion knowledge.
