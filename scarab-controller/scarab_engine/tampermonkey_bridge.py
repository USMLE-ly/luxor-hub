"""
Tampermonkey bridge — enhanced v2.0
Generates .user.js userscripts for Quora and X/Twitter automation.
"""
...
class TampermonkeyBridge:
    """Generate Tampermonkey userscripts."""

    def generate_script(self, platforms: list = None) -> str:
        if platforms is None:
            platforms = ["quora", "x"]
        parts = []
        if "quora" in platforms:
            parts.append(JS_QUORA)
        if "x" in platforms:
            parts.append(JS_X)
        return "\n\n".join(parts)

    def save_script(self, output_path: str = None) -> str:
        if not output_path:
            output_path = str(PROJECT_ROOT / "scarab_automation.user.js")
        with open(output_path, "w") as f:
            f.write(self.generate_script())
        print(f"[+] Tampermonkey script saved: {output_path}")
        return output_path


class ContentQueue:
    """Manage content queue for Tampermonkey bridge."""

    @staticmethod
    def save_queue(items: list, filepath: str = None) -> str:
        if filepath is None:
            filepath = str(PROJECT_ROOT / "content_queue.json")
        with open(filepath, "w") as f:
            json.dump(items, f, indent=2)
        print(f"[+] Content queue saved: {filepath} ({len(items)} items)")
        return filepath

    @staticmethod
    @staticmethod
    def generate_queue_script(items: list) -> str:
        encoded = base64.b64encode(json.dumps(items).encode()).decode()
        return (
            '// ==UserScript==\n'
            '// @name         SCARAB - Content Queue Import\n'
            '// @version      1.0\n'
            '// @description  Pre-populate SCARAB queue\n'
            '// @author       ZORG-OMEGA\n'
            '// @match        https://www.quora.com/*\n'
            '// @grant        GM_setValue\n'
            ...
            "        GM_setValue('scarab_quora_queue', JSON.stringify(data));\n"
            "        console.log('[SCARAB] Queue loaded:', data.length, 'items');\n"
            '    } catch(e) {\n'
            "        console.error('[SCARAB] Queue load failed:', e);\n"
            '    }\n'
            '})();\n'
        )
