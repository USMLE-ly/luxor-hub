  ...
  return (
    <div className="relative rounded-2xl">
      ...
      <div className="absolute -inset-px rounded-2xl overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-gold/15 via-transparent to-gold/8" />
        <motion.div
          className="absolute inset-0"
          style={{
            background: "conic-gradient(from 0deg, transparent, hsl(var(--gold) / 0.3), transparent, transparent)",
          }}
          animate={{ rotate: 360 }}
      ...
      {/* Inner card */}
      <div className="relative glass rounded-2xl p-8 backdrop-blur-xl bg-card/60">
        {/* Top gold radial glow */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-24 bg-gold/6 rounded-full blur-2xl pointer-events-none" />
        {children}
      </div>
    </div>
  );
